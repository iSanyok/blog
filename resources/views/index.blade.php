                    ...
                    <p>{{ $article->description }}</p>
                        <a href="{{ route('show', ['id' => $article->id]) }}" class="stretched-link" style="text-decoration: none; color: dimgray">
                            <div class="d-flex flex-nowrap bd-highlight align-self-end " style="position: absolute; bottom: 0; left: 10em">
                                <div class="order-1 p-2 bd-highlight"><small>{{ $article->updated_at->format('d.m.Y') }}</small></div>
                                <div class="order-3 p-2 bd-highlight"><small>Likes: {{ count($article->likes) }}</small></div>
                            </div>
                        </a>
                </div>
            </div>
    ...
    <div id="sidebar">
        ...
        <div class="row g-0 bg-light position-relative mb-3">
            ...
            <div class="col-md-6 p-4 ps-md-0" style="position: relative">
                ...
                <p>{{ $article->description }}</p>
                <a href="{{ route('show', ['id' => $article->id]) }}" class="stretched-link" style="text-decoration: none; color: dimgray">
                    <div class="d-flex flex-nowrap bd-highlight align-self-end " style="position: absolute; bottom: 0; left: 3em">
                        <div class="order-1 p-2 bd-highlight"><small>{{ $article->updated_at->format('d.m.Y') }}</small></div>
                        <div class="order-3 p-2 bd-highlight"><small>Likes: {{ $article->likes }}</small></div>
                    </div>
                </a>
            </div>
        </div>
